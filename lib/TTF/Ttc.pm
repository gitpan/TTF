...
TTF::Ttc - Truetype Collection class
...
An array of directories (TTF::Font objects) for each sub-font in the directory
...
sub read
{
    ...
    for ($i = 0; $i < $num; $i++)
    {
        ...
        $self->{'directs'}[$i] = TTF::Font->new('INFILE' => $fh,
                                                'PARENT' => $self,
